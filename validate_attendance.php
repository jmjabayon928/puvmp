...
if ( (strpos($ip_addr, $string) !== false) || $ip_addr == '::1' ) {
	$ecode = htmlentities(trim(real_escape_string($_POST['ecode'])), ENT_NOQUOTES);

	// handle the form
	if( !empty($ecode) ) {
		$sql = "SELECT eid, TIME_FORMAT(time_in, '%H:%i')
				FROM employees
				WHERE active = 1 AND (epass = '$ecode' OR ecode = '$ecode')";
		$res = query($sql); 
		$num = num_rows($res); 
		...
		if ($num > 0) {
			...
			if ($num > 0) {
				...
				if ( $am_in == 0 && $am_out == 0 && $pm_in == 0 && $pm_out == 0 ) {
					...
					if ( $sec_now < $intended_am_out ) {
						// update attendance
						$sql = "UPDATE attendance SET am_in = CURTIME(), status = -1 WHERE aid = '$aid'";
						if (@query($sql)) {
							// update time in attendance 
							update_time_in_attendance($aid); 
							// redirect to time_in.php
							header('location: time_in.php?eid='.$eid.'&success=1'); 
							exit(); 
						}
						...
						if (@query($sql)) {
							...
							update_time_in_attendance($aid); 
							// redirect to time_in.php
							header('location: time_in.php?eid='.$eid.'&success=2'); 
							exit(); 
						}
					}
					...
					if ( $sec_now <= $am_in ) { 
						...
						// redirect to time_in.php
						header('location: time_in.php?wrong_timein=2'); 
						exit(); 
					} elseif ( $intended_am_in < $sec_now && $sec_now < $intended_pm_in ) {
						// time in is between intended am in and intended pm in
						// update attendance
						$sql = "UPDATE attendance SET am_out = CURTIME(), status = -1 WHERE aid = '$aid'";
						if (@query($sql)) {
							// update time in attendance 
							update_time_in_attendance($aid); 
							// redirect to time_in.php
					...
					if ( $sec_now <= $pm_in ) { 
						// time in is less than or equal to recorded time in in the database
						// redirect to time_in.php
						header('location: time_in.php?wrong_timein=3'); 
						exit(); 
					} elseif ( $pm_in < $sec_now ) {
						// time in is greater than the recorded pm in
						// update attendance
						$sql = "UPDATE attendance SET pm_out = CURTIME(), status = 1 WHERE aid = '$aid'";
						if (@query($sql)) {
							// update time in attendance 
							update_time_in_attendance($aid); 
							// redirect to time_in.php
					...
					if ( $sec_now <= $am_out ) {
						...
						// redirect to time_in.php
						header('location: time_in.php?wrong_timein=4'); 
						exit(); 
					} elseif ( $sec_now < $intended_am_out ) {
						// time in is before the intended am out
						// redirect to time_in.php
						header('location: time_in.php?wrong_timein=5'); 
						exit(); 
					} elseif ( $sec_now > $intended_am_out && $sec_now < $intended_pm_out ) {
						...
						// forgot to time in in the afternoon
						// update attendance 
						$sql = "UPDATE attendance SET pm_out = CURTIME(), status = -1 WHERE aid = '$aid'";
						if (@query($sql)) {
							// update time in attendance 
							update_time_in_attendance($aid); 
							// redirect to time_in.php
							header('location: time_in.php?eid='.$eid); 
							exit(); 
						}
					}
					...
					if ( $sec_now <= $pm_in ) {
						...
						// redirect to time_in.php
						header('location: time_in.php?wrong_timein=6'); 
						exit(); 
				...
				if ( ($sec_now <= $intended_am_in) || ($intended_am_in < $sec_now && $sec_now < $intended_am_out) ) { 
					...
					// time in is between intended am out and intended pm out
					$sql = "INSERT INTO attendance( adate, pm_in, eid, status, tardiness ) VALUES( CURDATE(), CURTIME(), '$eid', -1, 240 )";
					if (@query($sql)) {
						$aid = mysqli_insert_id($connection);
						// update time in attendance 
						update_time_in_attendance($aid); 
						...
						// redirect to time_in.php
						header('location: time_in.php?eid='.$eid.'&success=3'); 
						exit(); 
					}
				} elseif ( $sec_now >= $intended_pm_out ) {
					// time in is on or later the intended pm out
					header('location: time_in.php?wrong_timein=1'); 
					exit(); 
				}
			}				
			...
			// redirect to time_in.php
			header('location: time_in.php?mismatched=2'); 
			exit(); 
		}
		...
		// redirect to time_in.php
		header('location: time_in.php?mismatched=1'); 
		exit(); 
	}
} else {
	// redirect to time_in.php
	header('location: time_in.php?wrong_timein=7'); 
	exit(); 
}
